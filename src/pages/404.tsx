...
const generateRandom = (min: number, max: number, length: number): number[] => {
  const minimum = Math.ceil(min);
  const maximum = Math.floor(max);
  const getRandomNumber = () =>
    Math.floor(Math.random() * (maximum - minimum + 1)) + min;
  return Array.from(new Array(length), () => getRandomNumber());
};

const x = generateRandom(-400, 400, 60);
const y = generateRandom(-400, 400, 60);
const scale = generateRandom(0, 1, 60);
const rotate = generateRandom(-0.5, 0.5, 60);
const opacity = generateRandom(0, 1, 60);
...
const Owl = styled(motion.svg).attrs(props => ({
  ...
  initial: {
    ...
    scale: 0,
    opacity: 0,
    fill: props.theme.colors.secondary9,
  },
  animate: {
    x,
    y,
...
const StyledText = styled(motion.p).attrs(() => ({
  variants: item,
